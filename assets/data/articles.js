window.ARTICLE_DATA = [
  // ===== TIN TỨC =====
  {
    id: "news-001",
    type: "news",
    isFeatured: true,
    title: "Giá xăng dầu tăng, có loại vượt 40.000 đồng/lít",
    author: "Ban Biên tập",
    date: "2026-04-03",
    category: "Thời sự",
    thumbnail: "assets/images/articles/bai-001.jpg",
    excerpt: "Liên Bộ Công Thương - Tài chính vừa điều chỉnh giá...",
    content: `<p>Nội dung chi tiết...</p>`
  },

  {
    id: "news-002",
    type: "news",
    isFeatured: true,
    title: "Luật Đô thị đặc biệt sẽ giúp TPHCM tăng trưởng vượt bậc",
    author: "Ban Biên tập",
    date: "2026-04-02",
    category: "Kinh tế",
    thumbnail: "assets/images/articles/bai-002.jpg",
    excerpt: "Chính sách mới giúp thành phố phát triển...",
    content: `<p>Nội dung...</p>`
  },

  // ===== CẢM NGHĨ (giữ nguyên) =====
  {
    id: "cam-xuc-001",
    title: "Tự hào tiếp bước truyền thống vẻ vang của Học viện Quốc phòng",
    author: "Tô Thị Thu",
    unit: "Cục HL-ĐT",
    date: "2026-03-20",
    category: "Cảm nhận",
    featured: true,
    thumbnail: "assets/images/articles/bai-001.jpg",
    excerpt: "Chặng đường 50 năm xây dựng và trưởng thành của Học viện Quốc phòng là niềm tự hào sâu sắc đối với mỗi cán bộ, giảng viên, học viên.",
    content: `
      <p>Trong dòng chảy 50 năm xây dựng, chiến đấu và trưởng thành, Học viện Quốc phòng đã khẳng định vị thế là trung tâm đào tạo, bồi dưỡng cán bộ cấp chiến dịch, chiến lược của Quân đội nhân dân Việt Nam.</p>
       <figure class="article-figure">
        <img src="assets/images/articles/bai-001-01.jpg" alt="Khai giảng năm học mới">
        <figcaption>Khoảnh khắc ghi dấu niềm tự hào trong các hoạt động kỷ niệm tại Học viện.</figcaption>
      </figure>
      <p>Mỗi dấu mốc phát triển của Học viện là kết tinh của trí tuệ, bản lĩnh, tinh thần đoàn kết và sự cống hiến bền bỉ của nhiều thế hệ cán bộ, giảng viên, học viên.</p>
      <p>Đối với chúng tôi, niềm tự hào không chỉ đến từ bề dày truyền thống mà còn từ trách nhiệm tiếp bước, giữ gìn và phát huy những giá trị tốt đẹp ấy trong giai đoạn mới.</p>
    `
  },
  {
    id: "cam-xuc-002",
    title: "Ký ức đẹp về ngôi trường mang tầm vóc chiến lược",
    author: "Trần Thị Hải Hà",
    unit: "Khoa Trinh sát",
    date: "2026-03-18",
    category: "Ký ức",
    featured: true,
    thumbnail: "assets/images/articles/bai-002.jpg",
    excerpt: "Mỗi hàng cây, giảng đường, mỗi giờ học và mỗi buổi sinh hoạt đều để lại trong tôi những ký ức đẹp, sâu đậm về Học viện Quốc phòng.",
    content: `
      <p>Học viện Quốc phòng không chỉ là nơi học tập, rèn luyện mà còn là nơi hun đúc bản lĩnh, phương pháp tư duy khoa học và tầm nhìn chiến lược cho mỗi học viên.</p>
      <p>Những năm tháng gắn bó với Học viện đã để lại trong tôi nhiều ký ức sâu sắc về tình đồng chí, đồng đội, về sự tận tâm của đội ngũ giảng viên, cán bộ quản lý và về môi trường quân sự chính quy, mẫu mực.</p>
      <p>Kỷ niệm 50 năm ngày truyền thống là dịp để mỗi người nhìn lại chặng đường đã qua với niềm xúc động, biết ơn và tự hào.</p>
    `
  },
  {
    id: "cam-xuc-003",
    title: "Phụ nữ Học viện Quốc phòng với niềm tự hào và trách nhiệm",
    author: "Trần Thị Thu Hiền",
    unit: "Hội Phụ nữ Học viện Quốc phòng",
    date: "2026-03-15",
    category: "Chia sẻ",
    featured: false,
    thumbnail: "assets/images/articles/bai-003.jpg",
    excerpt: "Mỗi hội viên phụ nữ hôm nay càng thêm tự hào khi được góp sức nhỏ bé của mình vào chặng đường phát triển bền vững của Học viện.",
    content: `
      <p>Trong hành trình phát triển của Học viện Quốc phòng, phụ nữ luôn có những đóng góp thiết thực, bền bỉ và đầy trách nhiệm trên nhiều lĩnh vực công tác.</p>
      <p>Niềm tự hào ấy trở thành động lực để chúng tôi tiếp tục nỗ lực, sáng tạo, hoàn thành tốt nhiệm vụ được giao, cùng xây dựng Học viện ngày càng chính quy, hiện đại.</p>
    `
  }
];
